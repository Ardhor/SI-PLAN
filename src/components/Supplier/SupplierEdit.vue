...
import { supplierDetail, updateSupplier } from '../../lip/api/SupplierApi.js';
...
async function fetchSupplier() {
    const response = await supplierDetail(token.value, id);
    const responseBody = await response.json();
    console.log(responseBody);

    if(response.status === 200){
        supplier.name = responseBody.data.name;
        supplier.no_telepon = responseBody.data.no_telepon;
    } else {
...
async function handleEdit(){
    const response = await updateSupplier(token.value, id, {
        name: supplier.name,
    ...
    if(response.status === 200){
        await alertSuccess("Edit Supplier successfully");
    } else {
        await alertError(responseBody.errors)
    }
}
