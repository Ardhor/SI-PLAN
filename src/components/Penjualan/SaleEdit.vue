...
import { addPenjualan, detailPenjualan, getPenjualan, UpdatePenjualan } from '../../lip/api/PenjualanApi.js'
...
async function handleEditPenjualan() {
    try {
        ...
        // Kirim ke API
        const response = await UpdatePenjualan(token.value, id, payload);
        const responseBody = await response.json()
        if (response.status === 201) {
            await alertSuccess("Edit Penjualan Successfully")
        } else {
...
async function fetchPenjualan() {
    const respone = await detailPenjualan(token.value, id);
    const responseBody = await respone.json();
    console.log(responseBody);

    if(respone.status === 200){
        penjualan.penjualanItems = responseBody.data.penjualanItems
        if (Array.isArray(responseBody.data.pembayaran)) {
            penjualan.pembayaran = responseBody.data.pembayaran[0] || {
                metodePembayaran: "Cash",
                statusPembayaran: "success",
                keterangan: ""
            }
        } else {
            penjualan.pembayaran = responseBody.data.pembayaran || {
                metodePembayaran: "Cash",
                statusPembayaran: "success",
                keterangan: ""
            }
...
<template>
    ...
    <div class="ml-20 mr-20 p-8 bg-zinc-800 rounded-lg shadow-md">
        ...
        <div class="flex justify-end">
            <button @click="handleEditPenjualan"
